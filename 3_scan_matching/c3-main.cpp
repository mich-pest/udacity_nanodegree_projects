...
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations){
    ...
    // 1. Align source with starting pose
    Eigen::Matrix4d starting_pose_matrix = transform3D(startingPose.rotation.yaw, startingPose.rotation.pitch, startingPose.rotation.roll, startingPose.position.x, startingPose.position.y, startingPose.position.z);
    PointCloudT::Ptr transformSource (new PointCloudT); 
  	pcl::transformPointCloud(*source, *transformSource, starting_pose_matrix);

	// 2. Define ICP object, set parameters and call align method
    pcl::IterativeClosestPoint<pcl::PointXYZ, pcl::PointXYZ> icp;
    icp.setInputSource(transformSource); // Not source directly!!
	...
	// 3. If converged mul for starting position T matrix
    if(icp.hasConverged()){
        std::cout << "ICP obtained transformation matrix: " << icp.getFinalTransformation() << std::endl;
        // startingPose casted to matrix and multiplied with ICP-obtained transformation matrix
        transformation_matrix = (icp.getFinalTransformation().cast<double>()) * starting_pose_matrix;
    }else{
        std::cout<< "WARNING: ICP not converged!" << std::endl;
    }
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
            ...
            Eigen::Matrix4d transformation_matrix = Eigen::Matrix4d::Identity();
            transformation_matrix = ICP(mapCloud, cloudFiltered, pose, 3);
			...
			// DONE: Change `scanCloud` below to your transformed scan: `transform_pose`
