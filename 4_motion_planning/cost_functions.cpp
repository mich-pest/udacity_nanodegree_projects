    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // DONE: Distance from circles to obstacles/actor
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  // DONE: distance between last point on spiral and main goal
  auto delta_x = spiral[n-1].x - main_goal.location.x;  
  auto delta_y = spiral[n-1].y - main_goal.location.y;  
  auto delta_z = spiral[n-1].z - main_goal.location.z;  
