...
void path_planner(vector<double>& x_points, vector<double>& y_points, vector<double>& v_points, double yaw, 
                  double velocity, State goal, bool is_junction, string tl_state, vector< vector<double> >& spirals_x, 
                  vector< vector<double> >& spirals_y, vector< vector<double> >& spirals_v, vector<int>& best_spirals){
...
int find_closest_point_index(vector<double> x_points_list, vector<double> y_points_list, double current_x_position, double current_y_position){

  int closest_point_idx = -1;
  double closest_point_dist = 1000.0;
  double dist = -1.0;
  for(int i = 0; i < x_points_list.size(); i++){
    dist = sqrt(pow((y_points_list[i] - current_y_position), 2) + pow((x_points_list[i] - current_x_position), 2));
  ...
  //cout << "returning closest point idx = " << closest_point_idx << " with distance = " << closest_point_dist << endl;
...
int main ()
{
  ...
  // initialize pid controller for throttle (Step 1)
  pid_throttle.Init(0.2,0.001,0.06,1.0,-1.0);

  // initialize pid controller for steer (Step 1)
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          //cout << "state of the car: (x, y, yaw) = (" << x_position << ", " << y_position << ", " << yaw << ")" << endl;
          ...
          // Find closest point (and its index) to the car
          int closest_idx = find_closest_point_index(x_points, y_points, x_position, y_position);
          ...
          **/
          ...
          /**
          * DONE (step 3): compute the steer error (error_steer) from the position and the desired trajectory
          **/

          // Since angle_between_points returns the angle between actual car position and next planned waypoint, this is the steering error
          ...
          **/
          ...
          /**
          * DONE (step 2): compute the throttle error (error_throttle) from the position ('velocity') and the desired speed ('v_points[v_points.size() - 1]')
          **/
          // Error obtained as the difference between 2 velocities:
          // reference (last point in the planned velocity trajectory) - actual velocity of the car (retrieved by the simulation)
          error_throttle = v_points[closest_idx] - velocity;
          
          // debug cout
          cout << "error_steer = " << error_steer << "; error_throttle = " << error_throttle << "; iteration = " << i << endl;
          fflush(stdout);
          ...
          **/
          ...
          //msg = "42[\"reset\",{}]"; // to reset car position
